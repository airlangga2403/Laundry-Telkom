package org.d3ifcool.laundrytelkom.ui.fiture.setrika
...
import org.d3ifcool.laundrytelkom.adapter.recyclerview.SetrikaAdapter
...
class SetrikaRecyclerViewActivity : AppCompatActivity() {
    private lateinit var binding: ActivitySetrikaRvBinding

    //    RV
    private lateinit var rvSetrika: RecyclerView
    private var list: ArrayList<Category> = arrayListOf()

    val titleActivity = "Setrika"
    var title: String = ""
    var address: String = ""
...
//        BINDING FROM SATUAN
        binding = ActivitySetrikaRvBinding.inflate(layoutInflater)
        setContentView(binding.root)

        rvSetrika = binding.rvSetrika
        rvSetrika.setHasFixedSize(true)
...
//        Get Data From Firebase
        rvSetrika.layoutManager = LinearLayoutManager(this)
        startRecyclerView()
...
//
    ...
    private fun startRecyclerView() {
        CalculatePriceSetrika.reference = FirebaseDatabase.getInstance().getReference("ui").child("setrika")
        CalculatePriceSetrika.reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    val listSetrikaAdapter = SetrikaAdapter(list)
                    rvSetrika.adapter = listSetrikaAdapter
                }
